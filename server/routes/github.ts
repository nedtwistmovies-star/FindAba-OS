...
githubRouter.get("/branches", async (req, res) => {
  ...
  try {
    ...
    const { status, details } = formatGithubError(error, repo, token);
    console.warn(`[GitBranches] Failed to fetch branches for '${repo}' (status ${status}): ${details}`);
    res.status(status).json({
      success: false,
...
githubRouter.get("/org-repos", async (req, res) => {
  ...
  try {
    ...
    console.warn(`[GitOrgRepos] Failed to fetch organization repos for '${org}' (status ${status}): ${details}`);
    res.status(status).json({
      success: false,
