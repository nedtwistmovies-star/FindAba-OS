...
import { UploadCloud, Loader2, Image as ImageIcon, CheckCircle2, RefreshCcw, Sparkles } from 'lucide-react';
    ...
    try {
      ...
      try {
        ...
        if (url) {
          ...
          // ✅ SAFE AI CALL
          if (hasAI() && base64) {
            generateImageCaption(base64, file.type)
              .then(() => console.debug("[AI] Caption generated"))
        ...
        console.warn("[Registry] Cloud sync failed, using local fallback:", cloudErr);

        // ✅ LOCAL FALLBACK
        await onUpload(base64, base64);
        setStatus('local');

        // ✅ SAFE AI CALL (fallback mode)
        if (hasAI() && base64) {
          generateImageCaption(base64, file.type)
            .catch((err) => console.warn("[AI] Caption failed:", err));
        }
  ...
  return (
    <div className={`space-y-3 ${className}`}>
      <div className="flex justify-between items-end px-1">
        ...
        {status === 'success' && (
          <span className="text-[8px] font-black text-aba-green uppercase flex items-center gap-1 animate-fade-in">
            <CheckCircle2 size={10} /> Cloud Synced
          </span>
        )}

        {status === 'local' && (
          <span className="text-[8px] font-black text-aba-gold uppercase flex items-center gap-1 animate-fade-in">
            <Sparkles size={10} /> Local Signal Active
          </span>
        )}
      ...
      <div className="relative group cursor-pointer h-44">
        <div className={`w-full h-full bg-slate-900/50 rounded-[2.5rem] border-2 border-dashed flex flex-col items-center justify-center overflow-hidden transition-all duration-500 relative z-0 ${
          status === 'error' ? 'border-red-500/50' : 'border-white/5 group-hover:border-aba-gold/50'
        }`}>
