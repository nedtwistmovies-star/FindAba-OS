...
const Signup: React.FC<SignupProps> = ({
  ...
  const handleSignup = async (
    ...
    try {
      ...
      const user = await signUpWithUsername(
        formData.username
          .toLowerCase()
          .trim(),
        formData.email,
      ...
      if (user) {
        ...
        setMessage(
          'Account created successfully! Welcome to FindAba.'
        );
    ...
    <div className="fixed inset-0 z-[6000] bg-[#002113] text-white flex flex-col font-sans overflow-y-auto animate-fade-in">
      ...
      <header className="p-8 flex items-center justify-between relative z-10">
        <button
          ...
          className="p-4 bg-white/5 rounded-2xl border border-white/10 active:scale-90 transition-all"
        >
          <ArrowLeft size={24} />
        </button>
        ...
        <Logo
          size={48}
          className="border-2 border-aba-gold/20 shadow-2xl"
        />
      </header>
        ...
        <div className="w-full max-w-sm space-y-12 animate-slide-up">
          ...
          {error && (
            <div className="p-5 bg-red-500/10 border border-red-500/20 rounded-2xl flex items-start gap-4">
              ...
              <p className="text-[9px] font-black uppercase text-red-400 tracking-widest">
                {error}
              </p>
            </div>
          )}
          ...
          {message && (
            <div className="p-5 bg-aba-green/10 border border-aba-green/20 rounded-2xl flex items-start gap-4">
              <ShieldCheck
                className="text-aba-green shrink-0"
                size={18}
              />

              <p className="text-[9px] font-black uppercase text-aba-green tracking-widest">
                {message}
              </p>
            </div>
          )}
          ...
          >
            ...
            <button
              ...
              className="w-full py-5 bg-white/10 text-white rounded-full font-black uppercase text-[11px] tracking-[0.3em] border border-white/10"
            >
              {otpSent
                ? 'OTP SENT'
                : 'SEND WHATSAPP OTP'}
            ...
            >
              {loading ? (
                ...
                <Zap
                  size={22}
                  className="text-aba-dark fill-current"
                />
              )}
      ...
      <footer className="p-12 text-center opacity-30 select-none grayscale shrink-0">
        <span className="text-[16px] font-black uppercase tracking-[1em]">
          SANDALSroyalle
        </span>

        <p className="text-[8px] font-black uppercase tracking-widest mt-4">
          FIDELITY GATEWAY HANDSHAKE V19.2
        </p>
      </footer>
    </div>
  );
