  ...
  const getStatusColor = (status: OrderStatus) => {
    switch (status) {
      case OrderStatus.PAID:
        return "text-aba-gold border-aba-gold/20 bg-aba-gold/5";
      ...
      case OrderStatus.DELIVERED:
        return "text-aba-green border-aba-green/20 bg-aba-green/5";
      case OrderStatus.RELEASED:
        return "text-aba-green border-aba-green/20 bg-aba-green/5";
      case OrderStatus.CANCELLED:
        return "text-red-500 border-red-500/20 bg-red-500/5";
      case OrderStatus.DISPUTED:
        return "text-red-500 border-red-500/20 bg-red-500/5 animate-pulse-red";
      default:
  ...
  if (loading) {
    return (
      <div className="flex-1 flex items-center justify-center">
        <div className="animate-spin text-aba-gold">
          <Shield size={40} />
        </div>
      </div>
    );
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 px-4 md:px-12">
          ...
          <div className="flex items-center gap-6">
            ...
            <div>
              <h3 className="text-xl font-black uppercase text-white">
                {profile?.full_name || "Verified Citizen"}
              </h3>
              <p className="text-[10px] font-black text-aba-gold uppercase tracking-[0.2em]">
        ...
        >
          <div className="flex items-center justify-between">
            ...
            <span className="text-[10px] font-bold text-aba-green uppercase tracking-widest">
              Active
            </span>
          </div>

          <div className="pt-6">
            <p className="text-[10px] font-black uppercase text-white/40 tracking-[0.3em] mb-2">
              Current Balance
            </p>
            <h2 className="text-4xl font-black text-white">
              ₦{profile?.total_paid || 0}.00
            </h2>
          </div>
        </motion.div>
        ...
        >
          ...
          <div className="pt-2">
            ...
            <p className="text-[10px] font-black uppercase text-white/40 tracking-widest mt-2">
              Industrial Signals In Transit
            </p>
          </div>
        </motion.div>
      </div>
            ...
            {orders.length > 0 ? (
              orders.map((order) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-4 w-full md:w-auto justify-between">
                    <span
                      className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest border shadow-sm flex items-center gap-2 ${getStatusColor(order.status)}`}
                    >
                      {order.status === OrderStatus.RELEASED && (
                        <CheckCircle2
                          size={12}
                          className="text-aba-green animate-check-reveal"
                        />
                      )}
                      {order.status}
                    </span>
                    <ChevronRight
                      size={20}
                      className="text-white/20 group-hover:text-aba-gold transition-all group-hover:translate-x-1"
                    />
                  </div>
                </div>
              ))
              ...
              <div className="py-24 text-center space-y-6 opacity-20 bg-white/5 rounded-[3rem] border border-dashed border-white/10">
                ...
                <p className="text-xs font-black uppercase tracking-[0.4em]">
                  No industrial signals recorded
                </p>
              </div>
            )}
          </div>
              ...
              <div className="space-y-12">
                <div className="flex items-center gap-6">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] font-bold text-aba-gold uppercase tracking-[0.4em]">
                      Reference #{selectedOrder.id.slice(-8)}
                    </p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-8">
                  <div className="space-y-2">
                    ...
                    <p className="text-lg font-bold text-white uppercase tracking-tight">
                      {(selectedOrder as any).merchant?.name ||
                        "Aba Industrial Node"}
                    </p>
                  </div>
                  <div className="space-y-2">
                    ...
                    <p className="text-lg font-bold text-aba-green uppercase tracking-tight">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="pt-1">
                      <span
                        className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest border shadow-xl ${getStatusColor(selectedOrder.status)}`}
                      >
                  ...
                  <div className="bg-aba-green/10 border border-aba-green/20 p-8 rounded-[2.5rem] space-y-6">
                    <div className="flex items-center gap-4 text-aba-green">
                      ...
                      <h4 className="text-sm font-black uppercase tracking-widest">
                        Release Escrow Signal
                      </h4>
                    </div>
                    <p className="text-xs font-medium text-white/60 leading-relaxed uppercase tracking-wider">
                      The merchant has confirmed delivery. By releasing funds,
                      you authorize the transfer of the industrial commitment
                      from escrow to the partner's wallet.
                    </p>
                    <button
                      onClick={async () => {
                        ...
                        try {
                          const { releaseEscrow } =
                            await import("../services/facesService");
                          ...
                          if (success) {
                            ...
                            addToast(
                              "Fidelity Link Established. Funds Released.",
                              "success",
                            );
                          }
                        } catch (e) {
                          addToast(
                            "Signal Interrupted. Retry Release.",
                            "error",
                          );
                        }
                      }}
