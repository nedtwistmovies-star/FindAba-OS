...
const Explore: React.FC<ExploreProps> = ({ businesses, onBusinessClick, favorites, onToggleFavorite, setView, loading = false }) => {
  ...
  return (
    <div className="flex-1 flex flex-col bg-[#020617] animate-fade-in overflow-hidden h-full">
      ...
      <div className="px-6 py-6 bg-black/40 backdrop-blur-2xl border-b border-white/5 sticky top-24 z-[1000] shadow-2xl space-y-4">
         <div className="flex justify-between items-center max-w-7xl mx-auto w-full">
            <div className="flex items-center gap-4">
               <button 
                 onClick={() => setView('discover')} 
                 className="w-12 h-12 flex items-center justify-center bg-white/5 rounded-2xl text-aba-gold hover:bg-aba-gold hover:text-aba-dark transition-all active:scale-90 border border-white/10"
               >
                 <ArrowLeft size={24} />
               </button>
               <div>
                  <h2 className="text-xl md:text-2xl font-black uppercase tracking-tighter text-white leading-none">City Registry</h2>
                  <p className="text-[8px] font-black text-aba-gold/60 uppercase tracking-[0.4em] mt-2">Verified Industrial Nodes</p>
               </div>
            </div>
            <div className="flex bg-white/5 p-1 rounded-2xl border border-white/10 shadow-inner">
                <button 
                  onClick={() => setViewMode('grid')} 
                  className={`px-4 py-2.5 rounded-xl transition-all ${viewMode === 'grid' ? 'bg-aba-gold shadow-2xl text-aba-dark' : 'text-white/40 hover:text-white/60'}`}
                >
                  <LayoutGrid size={18} />
                </button>
                <button 
                  onClick={() => setViewMode('map')} 
                  className={`px-4 py-2.5 rounded-xl transition-all ${viewMode === 'map' ? 'bg-aba-gold shadow-2xl text-aba-dark' : 'text-white/40 hover:text-white/60'}`}
                >
                  <MapIcon size={18} />
                </button>
            </div>
         ...
         <div className="max-w-7xl mx-auto w-full flex gap-3">
            <div className="relative flex-1 group">
               <Search className="absolute left-6 top-1/2 -translate-y-1/2 text-white/20 group-focus-within:text-aba-gold transition-colors" size={20} />
               <input 
                 placeholder="Search factory name or product..." 
                 className="w-full pl-14 pr-8 py-5 bg-white/5 border border-white/10 rounded-3xl text-sm font-bold outline-none focus:border-aba-gold/50 transition-all shadow-2xl text-white placeholder:text-white/20"
                 value={search}
            ...
            <IndustrialButton
               ...
               className="hidden md:flex bg-aba-gold/10 border-aba-gold/20 text-aba-gold hover:bg-aba-gold hover:text-aba-dark"
            >
               Verify Hub
            </IndustrialButton>
            ...
            <button 
              ...
              className={`px-6 rounded-3xl border transition-all flex items-center gap-3 relative active:scale-95 ${showFilters || activeFilterCount > 0 ? 'bg-aba-gold border-aba-gold text-aba-dark shadow-2xl' : 'bg-white/5 border-white/10 text-white/40 hover:border-white/20'}`}
            >
               <Filter size={18} />
               <span className="hidden md:inline text-[10px] font-black uppercase tracking-widest">Filters</span>
               {activeFilterCount > 0 && (
                 <span className="absolute -top-2 -right-2 w-5 h-5 bg-aba-red text-white text-[9px] font-black rounded-full flex items-center justify-center border-2 border-aba-dark shadow-2xl animate-pulse">
                   {activeFilterCount}
                 </span>
               )}
            </button>
         ...
         {!search && (
            <div className="max-w-7xl mx-auto w-full flex flex-wrap gap-2 px-1">
               {[
                 "Looking for a fashion designer in Aba?",
                 "Top shoemakers in Aba today",
                 "Best places to repair phones in Aba"
               ].map((suggestion, idx) => (
                 <button 
                   key={idx}
                   onClick={() => {
                     const term = suggestion
                       .replace('?', '')
                       .replace('Looking for a ', '')
                       .replace('Top ', '')
                       .replace('Best places to ', '')
                       .replace(' in Aba today', '')
                       .replace(' in Aba', '');
                     setSearch(term);
                   }}
                   className="text-[9px] font-black uppercase tracking-widest px-4 py-2 bg-white/5 border border-white/5 rounded-xl text-white/30 hover:bg-aba-gold/10 hover:border-aba-gold/30 hover:text-aba-gold transition-all"
                 >
                   {suggestion}
                 </button>
               ))}
