...
const BuyerPortal: React.FC<Props> = ({ userRole, verificationStatus, isExportReady, setView }) => {
  ...
  const handlePostSignal = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setShowForm(false);
      refreshSignals();
      addToast("Request posted successfully. Sellers are being notified.", "success");
    } catch (e) {
      addToast("Failed to post request. Please check your connection.", "error");
    } finally {
      setCreating(false);
    }
  };
  ...
  const handleInterestSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setInterestBrief('');
      refreshSignals();
      addToast("Your interest has been submitted. The buyer will review your profile.", "success");
    } finally {
      setCreating(false);
    }
  };
  ...
  const handleCloseSignal = async (id: string) => {
    if (!confirm("Close this request? This will remove it from the active list.")) return;
    setLoading(true);
    try {
        ...
        <div className="fixed inset-0 z-[5000] bg-black/95 backdrop-blur-xl flex items-center justify-center p-6">
           <div className="w-full max-w-md bg-white rounded-[3rem] shadow-2xl overflow-hidden animate-slide-up">
              <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-aba-dark text-white">
                 <h3 className="text-xl font-black uppercase tracking-tight">Submit Interest</h3>
              ...
              <form onSubmit={handleInterestSubmit} className="p-8 space-y-6">
                 ...
                 <div className="space-y-1">
                    <label className="text-[9px] font-black uppercase text-slate-400 tracking-widest ml-1">Your message</label>
           ...
           <div className="w-full max-w-2xl bg-white rounded-[3.5rem] shadow-2xl overflow-hidden animate-slide-up my-8">
              <div className="p-10 border-b border-slate-100 bg-slate-50">
                 <div className="flex justify-between items-start mb-6">
                    <div className="px-4 py-1.5 bg-aba-dark text-aba-gold rounded-full text-[9px] font-black uppercase tracking-widest">Active Request</div>
              ...
              <div className="p-10 space-y-10">
                 <div className="space-y-4">
                    <h4 className="text-[11px] font-black uppercase text-slate-400 tracking-[0.4em] flex items-center gap-3"><Activity size={16} /> Interested Sellers ({selectedInterests.length})</h4>
                    ...
                    <div className="space-y-4">
                       ...
                       {selectedInterests.length === 0 && (
                          <div className="py-12 text-center opacity-30 italic text-xs font-medium">Waiting for sellers to respond...</div>
                       )}
                    </div>
                 </div>

                 <div className="flex gap-4 pt-6 border-t border-slate-100">
                    <button onClick={() => handleCloseSignal(showDetailsModal.id)} className="flex-1 py-5 bg-aba-green text-white rounded-2xl font-black uppercase text-[10px] tracking-widest flex items-center justify-center gap-3 shadow-xl"><CheckCircle2 size={16} /> Close Request</button>
                    <button onClick={() => setShowDetailsModal(null)} className="flex-1 py-5 bg-slate-100 text-slate-400 rounded-2xl font-black uppercase text-[10px] tracking-widest">Cancel</button>
                 </div>
              </div>
           </div>
        </div>
           ...
           <div className="flex flex-col md:flex-row md:items-center justify-between gap-10">
              <div className="space-y-4">
                 <div className="inline-flex items-center gap-3 px-4 py-1.5 bg-aba-gold/10 border border-aba-gold/20 text-aba-gold rounded-full text-[9px] font-black uppercase tracking-widest shadow-2xl">
                    <Activity size={12} className="animate-pulse" /> Trade Center
                 </div>
                 <h2 className="text-6xl md:text-8xl font-black uppercase tracking-tighter leading-[0.85] text-white">Trade <br/><span className="text-aba-gold italic">Requests.</span></h2>
              </div>
              
              <div className="bg-white/5 p-1.5 rounded-3xl border border-white/5 flex shadow-2xl">
                 <button onClick={() => setActivePortal('discover')} className={`px-10 py-4 rounded-[1.3rem] text-[10px] font-black uppercase tracking-widest transition-all ${activePortal === 'discover' ? 'bg-aba-gold text-aba-dark shadow-xl' : 'text-white/40'}`}>All Requests</button>
                 <button onClick={() => setActivePortal('dashboard')} className={`px-10 py-4 rounded-[1.3rem] text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2 ${activePortal === 'dashboard' ? 'bg-aba-gold text-aba-dark shadow-xl' : 'text-white/40'}`}>My Requests {mySignals.length > 0 && <span className="px-2 py-0.5 rounded-full bg-white/20 text-white text-[8px] font-black">{mySignals.length}</span>}</button>
              </div>
           </div>
        </div>
      </div>
           ...
           <>
            <div className="bg-white p-10 rounded-[3rem] shadow-2xl border border-slate-100 flex flex-col md:flex-row items-center justify-between gap-10">
               <div className="flex items-center gap-8">
                  ...
                  <div>
                     <h4 className="text-xl font-black uppercase text-aba-dark tracking-tight leading-none">Verified Requests</h4>
                     <p className="text-[10px] font-black uppercase text-slate-400 tracking-[0.2em] mt-3">Status: Online and Secure</p>
                  </div>
               </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
                <div className="lg:col-span-2 space-y-8 pb-40">
                  {loading ? (
                    <div className="py-40 text-center"><Loader2 size={48} className="animate-spin text-aba-gold mx-auto mb-6" /><p className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Searching for requests...</p></div>
                  ) : signals.length === 0 ? (
                    <div className="py-40 text-center opacity-20 border-2 border-dashed border-slate-200 rounded-[4rem] flex flex-col items-center"><Database size={64} className="mb-6" /><h3 className="text-2xl font-black uppercase tracking-widest">No requests found</h3></div>
                  ) : (
                    signals.map(sig => {
                       ...
                       return (
                          <div key={sig.id} className="bg-white p-10 rounded-[4rem] shadow-xl border border-slate-100 flex flex-col md:flex-row items-center justify-between gap-10 group hover:border-aba-dark hover:shadow-2xl transition-all duration-700 relative overflow-hidden">
                             {isMyCategory && <div className="absolute top-0 right-0 px-6 py-2 bg-aba-gold text-aba-dark text-[7px] font-black uppercase tracking-[0.3em] rounded-bl-3xl shadow-xl">Good Match</div>}
                             ...
                             <div className="space-y-6 flex-1">
                                ...
                                <div className="flex flex-wrap gap-8 opacity-60">
                                   <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-slate-500"><Globe size={14} className="text-aba-dark" /> Destination: {sig.delivery_region}</div>
                                   <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-slate-500"><Handshake size={14} className="text-aba-dark" /> Offers: {sig.response_count} Seller{sig.response_count !== 1 ? 's' : ''}</div>
                                </div>
                             </div>
                             ...
                             <div className="w-full md:w-auto flex flex-col gap-3">
                                ...
                                <button onClick={() => addToast("Trade analytics require Premium Export Hub subscription.", "info")} className="text-[8px] font-black uppercase text-slate-300 tracking-widest hover:text-aba-dark transition-colors">Market Stats</button>
                             </div>
                          </div>
                       );
                    })
               ...
               <div className="space-y-10">
                  <div className="bg-white p-10 rounded-[3.5rem] border border-slate-100 space-y-10 shadow-xl">
                     <div className="flex items-center gap-4">
                        ...
                        <h3 className="text-sm font-black uppercase tracking-widest">Market Overview</h3>
                     </div>
                     <div className="space-y-8">
                        <div className="space-y-3">
                           <div className="flex justify-between items-end"><span className="text-[9px] font-black uppercase text-slate-400 tracking-widest">Total Requests</span><span className="text-[10px] font-black text-aba-dark">44.8M NGN</span></div>
                  ...
                  <div className="p-10 bg-gradient-to-br from-blue-600 to-blue-800 rounded-[3.5rem] text-white space-y-10 relative overflow-hidden group shadow-2xl">
                     ...
                     <div className="space-y-4 relative z-10">
                        <h4 className="text-2xl font-black uppercase tracking-tight">Global Reach</h4>
                        <p className="text-[10px] font-medium uppercase leading-loose tracking-widest text-blue-50/60">Verified sellers gain visibility to international buyers from 12 economic regions.</p>
                     </div>
           ...
           <div className="space-y-12 animate-slide-up pb-40">
              <div className="flex flex-col md:flex-row justify-between items-center gap-8">
                 <div className="space-y-2">
                    <h3 className="text-3xl font-black uppercase tracking-tight text-aba-dark">My Requests</h3>
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Manage your requests</p>
                 </div>
                 <button onClick={() => setShowForm(true)} className="px-10 py-6 bg-aba-dark text-white rounded-full font-black uppercase text-[10px] tracking-[0.4em] shadow-xl flex items-center gap-3"><Plus size={18} /> Post New Request</button>
              </div>
              ...
              <div className="grid grid-cols-1 gap-6">
                   ...
                   <div key={sig.id} className="bg-white p-10 rounded-[4rem] shadow-xl border border-slate-100 flex flex-col md:flex-row items-center justify-between gap-10 group transition-all hover:border-aba-dark">
                      ...
                      <div className="flex items-center gap-8">
                         <div className="text-center">
                            <p className="text-[9px] font-black uppercase text-slate-300 tracking-widest mb-1">Offers Received</p>
                 ...
                 {mySignals.length === 0 && (
                   <div className="py-40 text-center opacity-20 border-2 border-dashed border-slate-200 rounded-[4rem] flex flex-col items-center justify-center">
                      ...
                      <button onClick={() => setShowForm(true)} className="mt-8 px-8 py-4 bg-aba-dark text-white rounded-2xl text-[9px] font-black uppercase tracking-widest">Post a Request</button>
                   </div>
                 )}
              </div>
           </div>
           ...
           <div className="w-full max-w-xl bg-white rounded-[3.5rem] shadow-2xl overflow-hidden animate-slide-up my-8">
              <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-aba-dark text-white">
                 <div className="flex items-center gap-4">
                    ...
                    <div>
                       <h3 className="text-xl font-black uppercase tracking-tight leading-none">New Request</h3>
                       <p className="text-[8px] font-bold text-aba-gold uppercase tracking-widest mt-2">Secure Request Form</p>
                    </div>
                 </div>
              ...
              <form onSubmit={handlePostSignal} className="p-10 space-y-8">
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2">
                       <label className="text-[9px] font-black uppercase text-slate-400 tracking-widest ml-1">Urgency</label>
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                       <label className="text-[9px] font-black uppercase text-slate-400 tracking-widest ml-1">Payment Method</label>
                       <div className="flex bg-slate-50 p-1 rounded-2xl border border-slate-100 shadow-inner">
                    ...
                    <div className="space-y-2">
                       <label className="text-[9px] font-black uppercase text-slate-400 tracking-widest ml-1">Region</label>
                 ...
                 <div className="space-y-2">
                    <label className="text-[9px] font-black uppercase text-slate-400 tracking-widest ml-1">Request Details</label>
                    <textarea required rows={5} className="w-full p-6 bg-slate-50 border border-slate-100 rounded-[2rem] text-xs font-medium leading-relaxed outline-none focus:border-aba-gold resize-none shadow-inner" value={newSignal.requirement} onChange={e => setNewSignal({...newSignal, requirement: e.target.value})} placeholder="Describe what you need, including quality, sizes, and any other requirements..." />
                 </div>
