...
const HotelPartnerControl: React.FC<{ setView: (v: ViewState) => void }> = ({ setView }) => {
  ...
  const handleAuditSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setIsAuditing(false);
      refreshHotels();
      alert(`Audit Logged. Partner status updated. ${auditForm.score < 70 ? 'AUTOMATIC SUSPENSION TRIGGERED.' : ''}`);
  ...
  const handleAddRoom = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedHotel) return;
    await addRoomToPartner({ ...newRoomForm, hotel_id: selectedHotel.id, status: 'available' });
  ...
  if (loading && !selectedHotel && !isAddingNew) return (
    <div className="min-h-full bg-[#002113] flex flex-col items-center justify-center">
      ...
      <p className="text-[10px] font-black uppercase text-aba-gold tracking-[0.4em] mt-6">Scanning Partner Network...</p>
    </div>
  );
              ...
              <div className="p-8 border-b border-white/5 flex justify-between items-center bg-aba-dark">
                 <div>
                    <h3 className="text-xl font-black uppercase tracking-tight">
                      {isAuditing ? 'Quality Audit Hub' : (isEditingHotel || isAddingNew) ? 'Hotel Partner Profile' : 'Inventory Tagging'}
                    </h3>
                    <p className="text-[8px] font-bold text-aba-gold uppercase tracking-widest mt-1">
                      {isAddingNew ? 'Registering New Registry Partner' : `Target: ${selectedHotel?.name}`}
                    </p>
                 </div>
              ...
              <div className="p-8 max-h-[70vh] overflow-y-auto scrollbar-hide">
                 ...
                 {(isEditingHotel || isAddingNew) && (
                   <form onSubmit={handleUpdateHotel} className="space-y-6">
                      <div className="space-y-4">
                         <ImageUpload label="Partner Master Visual" currentImage={hotelForm.image_url} onUpload={(url) => setHotelForm({...hotelForm, image_url: url})} />
                         ...
                         <div className="grid grid-cols-2 gap-4">
                           <div className="space-y-1">
                              <label className="text-[8px] font-black uppercase text-white/60 tracking-widest ml-1">Partner Phone</label>
                      ...
                      <button type="submit" className="w-full py-6 bg-blue-600 text-white rounded-[2rem] font-black uppercase text-[10px] tracking-[0.3em] shadow-xl">{isAddingNew ? 'Provision New Partner' : 'Sync Partner Specifications'}</button>
                   </form>
                 )}
                 ...
                 {isManagingRooms && (
                   <div className="space-y-8">
                      <form onSubmit={handleAddRoom} className="p-6 bg-white/5 border border-white/10 rounded-[2rem] space-y-4">
                         ...
                         <button type="submit" className="w-full py-3 bg-white/10 hover:bg-white/20 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all">Register Room Unit</button>
                      </form>
      ...
      <div className="p-8 md:px-12 bg-aba-dark border-b border-white/5 flex items-center justify-between sticky top-0 z-50 backdrop-blur-xl bg-opacity-95">
        <div className="flex items-center gap-6">
          ...
          <div>
            ...
            <h2 className="text-2xl md:text-3xl font-black uppercase tracking-tighter leading-none">Partner <span className="text-aba-gold">Registry</span></h2>
          </div>
        </div>
      ...
      <div className="px-8 md:px-12 py-10 max-w-7xl mx-auto w-full space-y-12">
        <div className="flex flex-col md:flex-row gap-4 items-center justify-between bg-white/5 p-4 rounded-[2.5rem] border border-white/5 backdrop-blur-sm">
           <div className="relative flex-1 w-full">
              ...
              <input type="text" placeholder="Filter Partner Hubs..." className="w-full bg-white/5 border border-white/10 py-5 pl-14 pr-6 rounded-2xl text-[10px] font-black uppercase outline-none focus:border-aba-gold transition-all text-white" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
           </div>
...
export default HotelPartnerControl;
